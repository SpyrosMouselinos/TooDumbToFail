...
#model = VideoAudioFreqLearnMCVQA(train_mc_vqa_dataset)
...
model = VideoAudioFreqLearnMCVQA(train_mc_vqa_dataset)
#model.fit(val_external_dataset=val_mc_vqa_dataset, lr=0.001, bs=64)
results = {}
    ...
    for video_item in tqdm.tqdm(val_mc_vqa_dataset):
        ...
        for q_idx, q in enumerate(video_item['mc_question']):
            ...
            q_answer_id, q_answer = model.answer_q(a=a,
                                                   ...
                                                   sample=False,
                                                   temp=1,
                                                   use_gt_options=True, pre_softmax=True,post_softmax=False, approximate_gt_options=False)
            answer_dict = {
                'id': q['id'],
